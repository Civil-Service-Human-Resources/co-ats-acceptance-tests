...
import uk.gov.co.test.ui.pages.vx.VacancyDetailsPage.{vXReserveExtendLength, vXReserveListLength, vacancyId, vacancyName}
...
object ReserveListsTab extends VacancyBasePage {
  ...
  private def calculatedExpiryDate(monthsToAdd: Int): String = {
    ...
    if (monthsToAdd < 12) {
      ...
      val totalMonthsToAdd = monthsToAdd + vXReserveExtendLength.replaceAll("[A-Za-z ]", "").filterNot(_.isWhitespace).toInt
      val expiry = LocalDate.now().plusMonths(totalMonthsToAdd)
      val expiryDate = expiry.format(formatter)
      expiryDate
    }
  }
  ...
  def reserveExpiryDateMonths(): String =
    vXReserveListLength match {
      case "3 Months" => calculatedExpiryDate(3)
      case "6 Months" => calculatedExpiryDate(6)
      case "12 Months" => calculatedExpiryDate(12)
      case "15 Months" => calculatedExpiryDate(15)
      case "18 Months" => calculatedExpiryDate(18)
      case "21 Months" => calculatedExpiryDate(21)
      case "24 Months" => calculatedExpiryDate(24)
