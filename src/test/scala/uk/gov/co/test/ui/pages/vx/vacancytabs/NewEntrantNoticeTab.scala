...
import uk.gov.co.test.ui.pages.vx.ApplicationSummaryPage.{completeNewEntrantFormBarId, finaliseNenBarId, firstDayArrangementsAfterNenBarId, onboardingCompleteNen, onboardingFinaliseNen, passChecksBarId, requestUpdatedNenBarId}
...
object NewEntrantNoticeTab extends VacancyBasePage {
  ...
  private def moveToFinaliseNen(): Unit = {
  ...
  def newEntrantNoticeFlow(applicationDetails: ApplicationDetails): Unit = {
    ...
    moveToFinaliseNen()
    if (waitForVisibilityOfElementById(firstDayArrangementsAfterNenBarId).isDisplayed) println("NEN Form Complete")
  }
}
